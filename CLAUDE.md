...
| opus-4.8 | 4 | 8 | 8 | Default — when in doubt, opus; ~2x cheaper than fable and nearly as capable. Implementation that is ambiguous or exploratory, a large multi-file refactor, or a long agentic run stays here at `xhigh`: delegate it rather than editing inline on fable — sustained hands-on tool-driving (browser automation, QA sweeps, bulk extract/fill/snapshot) delegates the same way, not just code edits; a single gated go/no-go interaction can stay inline. Well-scoped, clearly-bounded implementation (terminal/shell-heavy execution included) routes to gpt-5.6-sol instead — sol executes bounded work faster and cheaper but drifts out of scope on open-ended tasks. |
...
| gpt-5.6-sol | 9 | 8 | 5 | Via the codex skill: code/diff review (finder and refuter passes), security review/audit and verification of security-sensitive code (auth, input validation, crypto, secrets — implementing that code stays in the fable lane), bug diagnosis, well-scoped or clearly-bounded implementation (edits, net-new code with crisp boundaries, and terminal/shell-heavy execution; ambiguous, large multi-file, or long-horizon work stays on opus), plateau second opinions, imagegen, rote throwaway scripts (gpt-5.6-luna may take the rote/bulk lane at the agent's discretion, and carries the sonnet-5 row's bounded recon sweeps at `xhigh` only — luna's `high` tier drops whole subsystems on deep traces; `service_tier=fast` stays pinned on every variant). A hard miss escalates to fable — ultra execution mode (exposed since codex 0.144.0) is not a retry rung: worst-in-class scope discipline at 3–5x cost. From workflows/subagents, `model` takes only Claude models — spawn the `codex:codex-wrapper` agent (ships with the codex plugin) with the self-contained question as its prompt; or invoke `Skill(codex)` directly (inline since codex plugin 0.10.0). |
...
**Effort**: `xhigh` by default; fable implementation may run `high`. `max` only after an xhigh attempt falls short. Verification runs at the same or higher model + effort tier than the work it checks; lanes the Models table routes to gpt-5.6-sol (refuter passes, security review/audit, verification of security-sensitive code) count as same-tier at `xhigh`, and sol-implemented code takes the same finder/refuter review as any other contributor's — escalation and findings synthesis stay on fable. The tier rule bites on un-routed choices: never verify opus work with sonnet.

...
15. **Bring the stack current with trunk** → `ccx vcs restack` (alias `rebase`; lane-selected like ship, gates included — the gt lane runs `gt sync`, jj fetches and rebases the working-copy stack onto `trunk()`, plain git fetches and rebases onto the remote default branch with `--autostash`; a conflict always refuses with the lane's recovery steps — `gt continue`/`gt abort`, a rolled-back jj rebase, an aborted git rebase — never headless resolution; `--no-gt` forces the jj/git lane)

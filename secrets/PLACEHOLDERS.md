...
| `@@VM_ADMIN_PASS@@` | Password for the baked-in admin user (sensitive) | `packer/bluebubbles.pkr.hcl`, `packer/metal.pkr.hcl` | generated by `scripts/lib/secrets.sh` → login Keychain (`yclaw-vm-admin-pass`, reused on re-run); packer reads it as `PKR_VAR_vm_admin_pass` |
| `@@BLUEBUBBLES_PASSWORD@@` | BlueBubbles server password | sops `hermes/env` → hermes VM; resolved into `scripts/bluebubbles-setup.sh` on the bluebubbles VM | generated by `scripts/lib/secrets.sh` → login Keychain (`yclaw-bluebubbles-password`, reused on re-run) → sops `hermes/env` |
...
| `@@AGENT_VAULT_MASTER_PASSWORD@@` | agent-vault vault encryption | vault server | generated by `scripts/lib/secrets.sh` → login Keychain (`yclaw-agent-vault-master`, reused on re-run) → sops `vault/master-password` |
...
- `@@VM_ADMIN_PASS@@` — `scripts/lib/secrets.sh` random-generates the admin password (`openssl rand`)
  into the login Keychain (`yclaw-vm-admin-pass`) and reuses it on re-run. It is NOT a sops/runtime
  secret: packer reads it from the Keychain as `PKR_VAR_vm_admin_pass` at image-build time (the
  `@@VM_ADMIN_PASS@@` token only ever appears as the packer var's fail-loud default).
- `@@BLUEBUBBLES_PASSWORD@@` — `scripts/lib/secrets.sh` random-generates it (`openssl rand`) into the
  login Keychain (`yclaw-bluebubbles-password`), reuses it on re-run, and renders it into the sops

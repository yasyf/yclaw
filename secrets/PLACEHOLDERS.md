...
| `@@CLIPROXY_API_KEY@@` | Inbound API key CLIProxyAPI enforces on metal:8317 (hermes presents it directly) | `darwin/metal-cliproxyapi-config.yaml`, `darwin/metal.nix`, `nixos/hermes.nix` | `secrets.sh` mints `openssl rand -hex 32` if unset, into sops `cliproxy/api-key` **and** `hermes/env` |
...
| `CLIPROXY_API_KEY` | resolves `@@CLIPROXY_API_KEY@@` (above) | `cliproxy/api-key`, `hermes/env` | minted by `secrets.sh` if unset |

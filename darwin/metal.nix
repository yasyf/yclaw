...
in
{
  ...
  system.activationScripts.postActivation.text = lib.mkMerge [
    ''
      ...
      # random-generated by scripts/lib/secrets.sh into the login Keychain (yclaw-vm-admin-pass) and
      # passed to packer via PKR_VAR_vm_admin_pass at image-build time (the packer var default is the
      # @@VM_ADMIN_PASS@@ placeholder), closing the cirruslabs base image's default admin/admin hole;
